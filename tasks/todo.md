...
## 任务（纵切，按依赖顺序）
- [ ] **D1** 配色令牌对齐 v6（BRAND→#234B3A + --green-2）+ 对比度测试 — S（地基）
- [ ] **D2** 首页重构到 v6：品牌头(移除余额/福利金) + 堂食/外卖分段(线描图标) + 扫码卡 + v1厨师 banner + 福利放送 — L（依赖 D1）
- [ ] **D3** 菜品页到 v6：插画菜品图(碗/盘/煲) + 购物车条压在 tab 之上 — L（依赖 D1）
- [ ] **D4** 下单页到 v6：福利金抵扣开关 + 支付方式仅微信 + 菜品缩略图 + 店头行 — M（依赖 D3）
- [ ] **D5** Tab 线描图标对齐 v6（墨绿激活）— S/M（依赖 D1）
