...
## Phase 3 — 测试覆盖
- [x] **T5** handler 覆盖缺口（依赖 T2）：limbo `shansong_status=0`→重派→400 表征测；重派成功写 `action="redispatch"` 审计断言；状态机白名单表测（合法 1→4/2→3/2→4 通过；非法 2→1/2→2/3→1/3→4/4→2/4→3→400 且状态不变）— S/M
- [x] **Checkpoint Complete** `go test ./...`（Redispatch `-count=5`）+ admin 全绿；重跑 `/ship` 期望 GO（仅剩已知风险）；端到端走查
