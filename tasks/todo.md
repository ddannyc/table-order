...
## 任务
- [ ] **T1** 规格差异/校验纯函数 `admin/src/utils/specSync.js`（`diffSpecs`/`validateSpecs`）+ `specSync.test.js` — S
- [ ] **T2** 编辑弹层内联「规格」草稿编辑区（展示+本地增删改，旧弹层暂留）— M
- [ ] **T3** 保存落库（菜品→规格 diff 复用既有接口）+ 删除旧独立规格弹层 + 「规格」列降级只读 — M
- [ ] **Checkpoint A** `npm test` 全绿 + `npm run build` 干净 + 手测四条链路 + git diff 仅含 `admin/`
