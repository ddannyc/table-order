...
### Phase 1 — 规格选择弹层重构
- [ ] **T1** 规格弹层交互逻辑：`openSpecPicker` 默认选首个在售 + `selectSpec`/`specInc`/`specDec`/`confirmAddSpec`，删按行 `pickSpec`；抽 `utils/spec.js`(`pickDefaultSpec`/`clampQty`) + `spec-picker.test.js` 单测 — S/M
