...
- [x] **T6** 入参校验：`shop_id/status` strconv→400、`type` 枚举、检查 Count/Scan/Find `.Error`；+ `page_size>100` 钳制测试 — S
- [x] **T7** 限流中间件（登录/注册 + redispatch，429，阈值可配）+ 测试 — M
- [x] **Checkpoint Complete** CI 全绿；ship 再评审 GO；端到端走查（已取消不可重派/限流/审计）；Ready for review
